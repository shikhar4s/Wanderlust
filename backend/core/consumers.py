...
class ChatConsumer(AsyncWebsocketConsumer):
 async def connect(self):
  ...
  await self.channel_layer.group_add(self.room,self.channel_name);await self.accept()
 ...
 async def receive(self,text_data):
  ...
  if body:
   msg=await self.save_message(body);await self.channel_layer.group_send(self.room,{'type':'chat.message','body':body,'sender':self.scope['user'].id,'created_at':msg.created_at.isoformat()})
 ...
 @database_sync_to_async
 def save_message(self,body):return Message.objects.create(conversation_id=self.conversation_id,sender=self.scope['user'],body=body)
